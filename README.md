# Manna

A private, reverent mobile app for Christians to process thoughts and struggles through AI-guided scriptural reflection.

## About

Users write a reflection ("Pouring"), and Manna returns a single NIV Bible verse with an empathetic explanation and closing prayer prompt. Built with privacy and minimalism at its core — no push notifications, no social features, no ads in sacred spaces.

## Tech Stack

- **Expo** (React Native) — iOS & Android
- **Claude API** — context inference and scriptural guidance
- **Supabase** — encrypted storage
- **YouVersion API** — NIV Bible verses

## Getting Started

```bash
npm install
npm start
```

Then press `i` for iOS simulator or `a` for Android emulator.

## Scripts

- `npm start` — Start Expo dev server
- `npm run ios` — Start on iOS
- `npm run android` — Start on Android
- `npm run web` — Start on web
